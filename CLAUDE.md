...
- `internal/telemetry` — runny's only OTEL importer (ADR-0024): installs OTLP trace + metric providers from `observability.otlp` config, resource attribution, bounded shutdown; installs nothing when the config block is absent. Also the trace emitter: folds `obs.Event` into a `runny.cycle` → `cycle.step` → `cycle.step.action` span tree with deterministic IDs.
