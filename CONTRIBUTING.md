...
**Renovate and the bzlmod lockfile:** hosted Renovate bumps `MODULE.bazel` (and the bazel version in `.mise.toml`) but cannot run bazel, so `MODULE.bazel.lock` arrives stale and CI's `--lockfile_mode=error` refuses the PR. That refusal is the guard working. To land such a PR: check out the Renovate branch, `bazel mod deps --lockfile_mode=update`, build/test, commit the lockfile to the branch, and merge once CI re-proves it. Bazel-version bumps need the same treatment (new versions resolve different implicit deps).
...
- **Workflows are zizmor-audited** (mise-managed, default persona): pre-commit audits staged workflow files offline; the `zizmor` CI job runs the full audit with online checks. Fix findings rather than suppress; a suppression needs an inline `# zizmor: ignore[rule]` comment whose justification survives review. GitHub App tokens (`create-github-app-token`) always declare `permission-*` inputs — an unscoped token inherits every permission the App installation has.
