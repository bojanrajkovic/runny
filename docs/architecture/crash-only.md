# Crash-only: no silent failure

Crash-only is the slot lifecycle property that makes a runner slot's failures
impossible to hide. A slot's life always resolves *visibly*: it makes forward
progress to a completed job, or it destroys the guest and recycles with a
recorded cause and a visible retry. There is no quiet in-place-repair mode for it
to limp in, and no path on which it records success or looks healthy while it is
in fact broken. This is the slot-level expression of the project's governing
trade-off — silent-failure-proofness over throughput.

This doc is the canonical definition the rest of the repo points at. The FSM that
enforces the property — the universal TEARDOWN sink, per-state deadlines, backoff
— is decided in
[ADR-0004](../architecture-decisions/0004-crash-only-state-machine.md), and its
living transition map is in [runnyd.md](runnyd.md). Two layers make the term
precise: the **mechanism** (destroy-and-recycle) and the **property** it buys (no
silent failure). Reaching for "crash-only" should mean one of them, named.
...
3. **A daemon restart is a cold start.** Guests are in-process VMs
   ...
   merely dies with the daemon — the app's connection FSM, the socket server — is
   not thereby crash-only. *Test:* nothing on disk is read back as live FSM state
   at startup.
...
the **live status reflects it truthfully** while it persists: a failing slot's
consecutive-failure streak is surfaced and is never laundered by a benign ending,
and an undead guest renders as a loud `WEDGED!`, never a clean teardown. The bar
is *never a success-record or healthy-status over a real failure* — the precise
lie an earlier shape told (recording teardown `ok` while a ghost guest ate the
guest cap) and the one this property exists to forbid. Actively paging a human is
a *monitoring* concern layered on top, not part of the guarantee.

## What crash-only is not

- **Not a spectrum.** A subsystem either has the property or it does not; there is
  no "more" or "most" crash-only. Exiting the process is not a *purer* form of the
  virtue, it is just the largest teardown.

- **Not a general doctrine** of simplicity or robustness. A decision is justified
  by one of the three clauses, named — not by invoking the word. Most often it is
  clause 3: a restart that must not interrupt a running job has to drain first,
  because the in-process guest dies with the daemon.
...
- **Not the single-use / hermeticity boundary.** That each cycle runs a fresh
  ...
  conflated, but it is a different guarantee: crash-only would still hold for a
  reused guest that was recycled visibly on failure.

- **Not the no-unbounded-operations invariant.** Crash-only forbids silent
  *repair*; [bounds.md](bounds.md) forbids silent *hang*. They are the two halves
  ...
  unreachable" wait is safe only because it is both visible (crash-only) and
  bounded (bounds).
