...
// the outer authorization tier and the operator entry the daemon stamps on the
// socket after creating it is the second (see docs/security.md). The stale-socket remove handles a
...
// reach coming from that separate stamp, not the listen call (that split is
