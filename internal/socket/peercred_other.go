...
// proving the ioctl works on the running host -- the probe is why this is a
// function value rather than a const, and a var so tests can pin both
// arming branches on any platform. Until then newOperatorGate reads this to
// keep the operator-revocation gate unarmed here: arming it anyway would
// deny every RPC unconditionally (peerID never resolves, and the gate fails
// closed on an unreadable identity by design -- see docs/security.md's
...
// baseline that security.md already documents as the primary tier.
...
// instead -- see peerCredSupported. The privileged verdict is likewise
// never minted by omission: an accidental future arming fails closed.
