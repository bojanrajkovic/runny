...
- **A differencing child's PARENT needs explicit VM group access granted, or a bare compute system's `Start` fails outright — and the CHILD needs read-write, not just read.** Confirmed against real hardware (issue #319): `Start` failed with `"The chain of virtual hard disks is inaccessible. The process has not been granted access rights to the parent virtual hard disk for the differencing disk."` Hyper-V's own tooling (`New-VM`, `Add-VMHardDiskDrive`) grants this automatically; a bare HCS compute system created via raw `CreateComputeSystem`/`Start` never does. `CreateDifferencing` calls `security.GrantVmGroupAccess` (read-only) on the parent and `security.GrantVmGroupAccessWithMask(child, security.AccessMaskAll)` on the child (both the vendored `internal/winhcs/security` binding, itself a from-scratch DACL-edit reimplementation of `vmcompute`'s own unexported `GrantVmGroupAccess`/`GrantVmGroupAccessWithMask` — see that file's own doc comment), immediately after `createDifferencing` succeeds. Read-only on **both** — what hcsshim's own `internal/computestorage` helper does, for its own container-base/diff-layer use case — reproduced the identical generic "Access is denied" on real hardware once the child was actually attached as a running VM's writable boot disk: the child is the disk every guest write lands on, and hcsshim's helper never needs that because its base/diff VHDs aren't a live VM's boot disk. Confirmed the mechanism precisely, not just the symptom: read-only reproduced with the daemon's own binary, with a brand-new never-used slot name, and independent of which directory the child lived in — widening the child's grant to Full Control (matching Microsoft's own documented manual ACL fix-up for reattached Hyper-V disks) was the only thing that fixed it. This was invisible to every existing unit test: they all run against `fakeBackend`, which never touches a real file's DACL — only a genuine differencing-disk boot surfaces it.
